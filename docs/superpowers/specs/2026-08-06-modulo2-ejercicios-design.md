...
**Nota sobre modales anidados:** React Native `<Modal>` presenta cada instancia como un view-controller nativo separado en iOS; tener dos `<Modal>` con `visible=true` al mismo tiempo ya nos causó un bug real (la foto dentro del historial de Físico se abría "atrás" del modal de historial). Por eso, todo lo que se abre DESDE DENTRO de `ListaEjerciciosModal` (crear ejercicio, editar objetivo de carpeta, registrar sesión) es una vista interna de ese mismo `<Modal>`, no un `<Modal>` propio — mismo patrón lista↔detalle ya usado en `HistorialObjetivosModal`/`HistorialFisicoModal`, con más vistas posibles.

- Reemplazar `src/screens/EjerciciosScreen.js`: fila fija "Todos los ejercicios" + lista de carpetas (nombre + cantidad de ejercicios), botón "+ Nueva carpeta" (mismo estilo de icono-botón que Objetivo/Físico) que abre `CrearCarpetaModal` (este sí es un `<Modal>` propio — se abre directo desde la pantalla, no desde otro modal).
- Crear `src/components/CrearCarpetaModal.js`: un solo input de texto (nombre) + Guardar/Cancelar.
...
- Crear `src/components/ListaEjerciciosModal.js`: un único `<Modal>` con una vista interna controlada por estado (`'lista' | 'detalle' | 'crear' | 'editarObjetivo' | 'registrarSesion'`):
  - `'lista'`: ejercicios de la carpeta (o todos), cada fila con imagen/emoji + nombre + tipo. Si se abrió desde una carpeta (no "Todos"), cada fila tiene un botón chico "Editar en esta carpeta" que cambia la vista a `'editarObjetivo'`. Botón "+ Crear ejercicio" cambia la vista a `'crear'`.
  - `'detalle'`: imagen, descanso de referencia, gráfica, historial de sesiones, botón "Registrar sesión" que cambia la vista a `'registrarSesion'`.
  - `'crear'`: formulario con nombre, grupo muscular, toggle tipo (peso/reps o tiempo), descanso en segundos (opcional), selector de foto (opcional, mismo patrón que `RegistrarFisicoModal`), checkboxes de carpetas (multi-selección, con la carpeta actual preseleccionada si corresponde). Sin orden ni objetivo — eso se completa después, desde `'editarObjetivo'`.
  - `'editarObjetivo'`: input numérico de orden, series objetivo, y reps objetivo o duración objetivo según `exercise.type`. Guarda vía `updateFolderItem`.
  - `'registrarSesion'`: si el ejercicio tiene objetivo en la carpeta actual, pre-completa cantidad de series y cada fila con `target_reps`/`target_duration_seg` (editable); si no, arranca vacío. Selector de cantidad de series (1-8), filas de peso+reps (o duración), fecha automática, guarda vía `createExerciseLog`.
  - Cada vista tiene su propio botón "‹ Volver" a la vista anterior lógica (mismo patrón de header que `HistorialObjetivosModal`).
...
- Crear `src/components/DiagramaMusculo.js`: muestra la foto propia si existe (con `getSignedUrl`), si no, el emoji de `muscle_group` (mapeo fijo, sin assets nuevos). Sin `<Modal>` propio — es un componente presentacional simple.
